...
void runAssembler(Assembler* asm) {
    ...
    for (size_t i = 0; i < asm->table->size; i++) {
        if (FUNCTION_TAG == asm->table->values[i]->tag) {
            addSymbol(asm->_obj, asm->table->keys[i], valueC++, TEXT_SECTION_NUM, 0, IMAGE_SYM_CLASS_LABEL, 0);
        }
    }
    addSymbol(asm->_obj,"_main", 0xE, TEXT_SECTION_NUM, 0, IMAGE_SYM_CLASS_LABEL, 0);
    ...
    setHeaders(asm->_obj, COMPUTER_TYPE, 3, 0, 0, asm->_obj->symbolTableSize, 0, 0x0105);
